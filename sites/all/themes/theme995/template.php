...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme995
 */

// Auto-rebuild the theme registry during theme development.
if ( theme_get_setting( 'theme995_clear_registry' ) ) {
	// Rebuild .info data.
 ...
 */
function theme995_html_head_alter( &$head_elements ) {
 ...
 */
function theme995_preprocess_search_block_form( &$vars ) {
 ...
 */
function theme995_preprocess( &$vars, $hook ) {
	$vars['theme995_path'] = base_path() . path_to_theme();
 ...
 */
function theme995_preprocess_html( &$vars ) {
	$vars['doctype'] = _theme995_doctype();
	$vars['rdf'] = _theme995_rdf( $vars );
	...
	if ( !$vars['is_front'] ) {
		...
		if ( isset( $page_name ) ) {
			$vars['classes_array'][] = theme995_id_safe( 'page-' . $page_name );
		}

		$vars['classes_array'][] = theme995_id_safe( 'section-' . $section );
	...
	if ( ( $d['destination'] == 'blog' ) && ( theme_get_setting( 'theme995_blog_title' ) != "" ) ) {
		$vars['title'] = theme_get_setting( 'theme995_blog_title' );
		$vars['head_title'] = theme_get_setting( 'theme995_blog_title' );
	}
}
 ...
 */
function theme995_preprocess_page( &$vars ) {
 ...
 */
function theme995_preprocess_maintenance_page( &$vars ) {
	...
	drupal_add_css( drupal_get_path( 'theme', 'theme995' ) . '/css/maintenance-page.css' );
}
 ...
 */
function theme995_preprocess_node( &$vars ) {
	// Striping class
 ...
 */
function theme995_preprocess_block( &$vars, $hook ) {
 ...
 */
function theme995_menu_tree( $vars ) {
 ...
 */
function theme995_field__taxonomy_term_reference( $vars ) {
	$output = '';
 ...
 */
function theme995_breadcrumb( $vars ) {
	$breadcrumb = isset( $vars['breadcrumb'] ) ? $vars['breadcrumb'] : array();

	$condition = theme_get_setting( 'theme995_breadcrumb_hideonlyfront' ) ? count( $breadcrumb ) > 1
			: !empty( $breadcrumb );
	$separator = theme_get_setting( 'theme995_breadcrumb_separator' );

	if ( theme_get_setting( 'theme995_breadcrumb_showtitle' ) ) {
 ...
 * @return bool
 */
function theme995_tabs_float() {
	$float = (bool) theme_get_setting( 'theme995_tabs_float' );
	$float_node = (bool) theme_get_setting( 'theme995_tabs_node' );
...
function theme995_id_safe( $string ) {
	// Strip accents
 ...
 * @ingroup themeable
 */
function theme995_menu_link( array $vars ) {
	...
	$element['#attributes']['class'][] = theme995_id_safe( $element['#title'] );
 ...
 */
function theme995_preprocess_menu_local_task( &$vars ) {
 ...
 */
function theme995_menu_local_tasks( &$vars ) {
	$output = '';
 ...
 */
function theme995_preprocess_comment( &$variables ) {
...
/* Region options */
function theme995_region_preffix ( $reg ) {
	$block_bg_type = theme_get_setting( 'theme995_' . $reg . '_bg_type' );
	$block_bg_img = theme_get_setting( 'theme995_' . $reg . '_bg_img' );
	$block_bg_parallax = theme_get_setting( 'theme995_' . $reg . '_bg_parallax' );
	$bg_video = theme_get_setting( 'theme995_' . $reg . '_bg_video' );
	$bg_video_start = theme_get_setting( 'theme995_' . $reg . '_bg_video_start' );
	$fullwidth = theme_get_setting( 'theme995_' . $reg . '_fullwidth' );
	...
	$output_preffix = '<div id="' . $reg . '_wrapper"' . $classes . $styles . $attributes . '><div class="region-before"></div>' . $content;
		if ( !$fullwidth ) {
...
function theme995_region_suffix ( $reg ) {
	$fullwidth = theme_get_setting( 'theme995_' . $reg . '_fullwidth' );
	$block_bg_parallax = theme_get_setting( 'theme995_' . $reg . '_bg_parallax' );
	$block_bg_img = theme_get_setting( 'theme995_' . $reg . '_bg_img' );
	...
	$output_suffix .= '<div class="region-after"></div></div>';

	print $output_suffix;
}


function theme995_form_contact_site_form_alter( &$form, &$form_state, $form_id ) {
